...
export default function ReviewerBoardPage() {
  ...
  const fetchMembers = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/reviewer-board');
      if (res.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingMember 
        ? `/api/reviewer-board/${editingMember.id}` 
        : '/api/reviewer-board';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/reviewer-board/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        fetchMembers();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto min-h-screen bg-slate-950 text-white">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold">Reviewer Board</h1>
          <p className="text-slate-400 mt-2">Manage the reviewer board members</p>
        </div>
        <button 
      ...
      {loading ? (
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-12 text-center">
          ...
          <p className="text-slate-400">Click the Add Reviewer button to create one.</p>
        </div>
      ) : (
            ...
            <div key={member.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col">
              <div className="p-6 flex flex-col items-center flex-grow text-center">
                ...
                {member.specialization && (
                  <div className="mt-1 text-xs bg-indigo-900/40 text-indigo-300 px-3 py-1 rounded-full">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center sticky top-0 bg-slate-900 z-10">
              <h2 className="text-2xl font-bold">{editingMember ? 'Edit Reviewer' : 'Add Reviewer'}</h2>
