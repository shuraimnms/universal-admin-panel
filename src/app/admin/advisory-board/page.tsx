...
export default function AdvisoryBoardPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingMember 
        ? `/api/advisory-board/${editingMember.id}` 
        : '/api/advisory-board';
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/advisory-board/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        fetchMembers();
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto min-h-screen bg-slate-950 text-white">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold">Advisory Board</h1>
          <p className="text-slate-400 mt-2">Manage the advisory board members</p>
        </div>
        <button 
